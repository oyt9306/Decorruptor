...
from diffusers import ConsistencyDecoderVAE, LCMScheduler
from diffusers import DDIMScheduler

import utils
import text2video.gradio_utils
import os
...
class Model:
    def __init__(self, device, dtype, **kwargs):
        ...
        self.pipe_dict = {
            ModelType.Pix2Pix_Video: ConsistInstructPix2PixPipeline,
            ...
            ModelType.Pix2Pix_LCM_Video: IP2PLatentConsistencyModelPipeline,
        }
        self.controlnet_attn_proc = utils.CrossFrameAttnProcessor(
            unet_chunk_size=2)
        self.pix2pix_attn_proc = utils.CrossFrameAttnProcessor(
            unet_chunk_size=3)
        self.lcm_pix2pix_attn_proc = utils.CrossFrameAttnProcessor(
            unet_chunk_size=1)
    ...
    def inference_chunk(self, frame_ids, **kwargs):
        ...
        # print(prompt[frame_ids].tolist(), negative_prompt[frame_ids].tolist(), latents, self.generator)
    ...
    def inference(self, split_to_chunks=False, chunk_size=8, lcm_mode=None, **kwargs):
        if not hasattr(self, "pipe") or self.pipe is None:
            return
    ...
    def process_pix2pix(self,
        ...
        if model == 'DPM':
            ...
            self.pipe = ConsistInstructPix2PixPipeline.from_pretrained(model_id, 
                                                                    torch_dtype=torch.float16, 
                                                                    safety_checker=None,
                                                                    use_safetensors=True)
            image_guidance_scale = list(np.sqrt(np.linspace(1.8**2,0.0**2,20))) #+ list(np.zeros(5))
            ...
            print("Module LCM-Pix2Pix")
            # vae = ConsistencyDecoderVAE.from_pretrained("openai/consistency-decoder", torch_dtype=torch.float16)
                                                                    # vae=vae,
            scheduler = LCMScheduler.from_pretrained(model_id, subfolder="scheduler")
            self.pipe = IP2PLatentConsistencyModelPipeline.from_pretrained(model_id, 
                                                                    torch_dtype=torch.float16,
                                                                    scheduler=scheduler, 
                                                                    safety_checker=None,
                                                                    use_safetensors=True)
            if use_cf_attn:
                self.pipe.unet.set_attn_processor(
                    processor=self.lcm_pix2pix_attn_proc)
